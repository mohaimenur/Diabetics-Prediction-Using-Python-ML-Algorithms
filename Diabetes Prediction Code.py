...
"""
# missing value fill up with mean value
# imp=SimpleImputer(missing_values=np.nan, strategy="mean")
# Imputer=imp.fit(x[:,0:8])
# x[:,0:8]=imp.fit_transform(x[:,0:8])

"""

"""
# When a dataset belongs string type data 
# label the data to machine readable form

# for x
le_x=LabelEncoder()
x[:,column number]=le_x.fit_transform(x[:,column number])

# for y
le_y=LabelEncoder()
y[:,column number]=le_y.fit_transform(y[:,column number])

# or,

# we can use onehotencoder
ohn_x=ColumnTransformer([("name of the column",OneHotEncoder(),[column number])],remainder="passthrough")
x=ohn_x.fit_transform(x)
